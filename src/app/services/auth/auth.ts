    ...
    validateRequestToken(username:string, password:string ) {
        return new Promise((resolve, reject) => {
            this.requestToken().then(
                data => {
                    this.http.get(this.config.urlApi + 'authentication/token/validate_with_login?api_key=' + this.config.apiKey + '&username=' + username + '&password=' + password + '&request_token=' + data['request_token']).toPromise().then(
                        response => resolve(response.json()),
                        err => reject(err)
                    );
                }, err => reject(err)
